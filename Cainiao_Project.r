...
result = mdat %>% group_by(merchant_id) %>% 
  summarize(avguv=mean(pcuv)) %>% arrange(desc(avguv))

m42 = mdat %>% filter(merchant_id==42) %>% 
  select(day,avgLogisticScore,avgServiceQualityScore) %>% arrange(day)
qplot(m42$day,m42$avgLogisticScore)
qplot(m42$day,m42$avgServiceQualityScore)
m323 = mdat %>% filter(merchant_id==323) %>% 
  select(day,avgLogisticScore,avgServiceQualityScore) %>% arrange(day)
qplot(m323$day,m323$avgLogisticScore)
qplot(m323$day,m323$avgServiceQualityScore)

# What can we learn about Merchant 42 and 323 from order data
odat %>% filter(merchant_id==42) %>% group_by(if_cainiao) %>% 
  summarize(norder=n_distinct(order_id))
odat %>% filter(merchant_id==323) %>% group_by(if_cainiao) %>% 
  summarize(norder=n_distinct(order_id))

# Now, does order fulfillment time decrease when Cainiao service was used?
# First, how can we calculate order fulfillment time?
# Fulfillment time = SIGNED (in logistics data) - pay_timestampe (in order data)
signed_data = ldat %>% filter(action=="SIGNED") %>% 
...
# Now, let's compare
odat %>% group_by(if_cainiao) %>% 
  summarize(avgTotalTime=mean(totaltime)) # about 10 hours faster!

# But... really? 10 hours?
filtered_odat = odat %>% filter(promise==0)
filtered_odat %>% group_by(if_cainiao) %>% 
  summarize(avgTotalTime=mean(totaltime)) # Still faster!

# Let's look deeper
odat %>% group_by(if_cainiao,promise) %>% 
  summarize(norder=n_distinct(order_id)) # Hmm!

# Are we done? Is that it? What should we do now?
# what can we do more?

# for example, we can investigate
lcdat = ldat %>% select(order_id,logistic_company_id) %>% 
  distinct()
result = lcdat %>% group_by(logistic_company_id) %>% 
  summarize(norder=n_distinct(order_id)) %>% arrange(desc(norder))
sum(result$norder) # the first two LCs cover 74% of total orders! (we have 38 LCs)
result = result %>% 
  mutate(LCsize=ifelse(norder>200000,"Large","Small"))

odat = odat %>% left_join(lcdat)
odat = odat %>% left_join(result)
head(odat)

odat %>% group_by(LCsize,if_cainiao) %>% 
  summarize(Avgtotaltime=mean(totaltime)) # 10 hrs vs. 7.6 hrs

last_city = ldat %>% filter(action=="SIGNED") %>% 
  select(order_id,city_id)
result = last_city %>% group_by(city_id) %>%
  summarize(norder=n_distinct(order_id)) %>% arrange(desc(norder))
